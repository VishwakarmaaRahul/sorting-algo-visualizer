...
function displayTimeTaken(time, complexity) {
    const timeDisplay = document.getElementById('time-display');
    timeDisplay.textContent = `Time Taken: ${time.toFixed(4)} seconds | Complexity: ${complexity}`;
}
...
async function bubbleSort() {
    ...
    displayTimeTaken((end - start) / 1000, "O(n^2)");
}
...
async function mergeSort() {
    ...
    const end = performance.now();
    displayTimeTaken((end - start) / 1000, "O(n log n)");
}
